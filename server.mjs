...
// 修复反向代理处理过的路径（仅在未编码且形如 http(s):// 时才重新编码）
app.use('/proxy', (req, res, next) => {
  try {
    const raw = req.url.replace(/^\//, '');
    if (/^https?:\/\//i.test(raw)) {
      // 未编码的完整 URL，修正可能缺少的第二个斜杠并进行一次编码
      const fixed = raw.replace(/(https?:)\/([^/])/, '$1//$2');
      req.url = '/' + encodeURIComponent(fixed);
    } else if (/^https?%3A/i.test(raw)) {
      // 已经是编码后的 URL，保持不变
      // 不做任何处理，避免二次编码
    } else {
      // 其他情况（例如路径异常），保持原样，交由下游处理
    }
  } catch (e) {
    // 出错也不要中断请求链路
  }
